...
- **DPO's failure mode is consistent across evaluation rounds: confident, specific, wrong additions.** Q1 (wrong species), Q5 (implausible distance), Q6 (real species name misapplied) all follow the same pattern — DPO tends to add detail and confidence without a proportional increase in accuracy, and sometimes decreases accuracy relative to SFT.
- **A specific, reproducible regression appears at Q7 in both SFT and DPO**: both invent parental incubation roles ("both parents," "males guarding the nest") that directly contradict the instruction dataset's explicit statement that males don't participate in nest care. This is not a case of a fact failing to transfer — it's the model confidently stating the opposite of what it was trained on, likely because the base model's strong general prior (many bird species do share incubation duties) is overriding the fine-tuning signal on this specific, narrower fact. This is worth targeted follow-up: consider adding more repetitions of this fact to the training data, or explicitly contrasting it against the general "many birds share incubation" pattern.
